...
CFactoryTemplate g_Templates[] = {
    { L"Antigravity iOS Cam", &CLSID_AntigravityCam, CVCam::CreateInstance, NULL, &sudVCam }
};
...
CVCam::CVCam(LPUNKNOWN lpunk, HRESULT *phr) : CSource(NAME("Antigravity Cam"), lpunk, CLSID_AntigravityCam) {
...
CVCamStream::CVCamStream(HRESULT *phr, CVCam *pParent, LPCWSTR pPinName) : CSourceStream(NAME("Output"), phr, pParent, pPinName) {
    m_hMapFile = NULL;
    m_pSharedMem = NULL;
...
void CVCamStream::InitSharedMemory() {
    ...
    if (m_hMapFile) {
        m_pSharedMem = (SharedMemoryLayout*)MapViewOfFile(m_hMapFile, FILE_MAP_READ, 0, 0, sizeof(SharedMemoryLayout));
    }
}
...
HRESULT CVCamStream::FillBuffer(IMediaSample *pms) {
    ...
    if (m_pSharedMem && m_pSharedMem->magic == 0x43424557) {
        // Simple polling approach (blocking here would block the graph, but FillBuffer is called in a loop)
        // ideally we wait for event, but for MVP we just copy latest
